...
def draw_corners(image, coords):
    """
    It draws a circle at each coordinate in the list of coordinates

    :param image: The image to draw the corners on
    :param coords: The coordinates of the corners
    """
    for c in coords:
...
def binarize_chessboard(image):
    """
    It takes an image of a chessboard and returns a binary image with only the squares of the chessboard

    :param image: the image to be binarized
    :return: The chessboard squares are being returned.
    """

    # Extrapolate edges via Canny
    ...
    # Select only the chessboard (assuming it's the biggest contour)
    max_area = 0
    max_contour = None
    ...
    # Filter only the squares of the chessboard
    ...
    # Morphological open to separate the clustered squares
...
def find_edges_automatically(image, camera_i):
    """
    It finds the contours of the chessboard and selects the
    contours coordinates corresponding to the corners of the chessboard

    :param image: the image to be binarized
    :param camera_i: the camera number (1, 2, 3, or 4)
    :return: the coordinates of the corners of the chessboard.
    """

    # indexes of the squares contours for each camera
    ...
    print(f"Edges coordinates found automatically: {corners_coords}")
...
for camera_i in range(1, 5):
    ...
    # Draw the axis for the intrinsics.avi file
        ...
        if retF:
            ...
            if retC:
                ...
                if camera_i != 3:
                    ...
                    object_points_intrinsics.append(op_cam3)
            print(f"{i}: {retC}")
            i += 1
        else:
            break
    ...
    if camera_i != 3:
        ...
        retval_extr, rvec_extr, tvec_extr = cv.solvePnP(
            op_cam3, corners, camera_matrix, dist_coeffs, flags=cv.SOLVEPNP_ITERATIVE)
    ...
    show_image(frame, f"Intrinsics Calibration for cam{camera_i}")
    ...
    while cap.isOpened():
        ...
        if retF:
            if camera_i != 3:
                ...
                retC, corners = cv.findChessboardCorners(
                    frame, (6, 8))
            if not retC:  # manual edges annotation
                ...
                show_image(cv.drawChessboardCorners(
                    frame_copy, (CHESSBOARD_VERTICES[0], CHESSBOARD_VERTICES[1]), corners, True), f"Extrinsics Calibration for cam{camera_i}")

            image_points_extrinsics.append(corners)
            object_points_extrinsics.append(op)
        break  # only use the first frame
    cap.release()
    ...
    # project camera 1 and 2 locations using camera 3 [extra]
    if camera_i == 3:
        ...
        show_image(frame, "Cam1 and Cam2 positions seen from Cam3")
