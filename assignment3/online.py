...
def get_color_model():
    ...
    for i_label, vox in enumerate(visible_voxels):
        ...
        pixels_colors.append(((x, y), labels[i_label][0], frames[1][0][y, x]))
...
def start_online(MGGs):
    ...
    for n_frame in range(0, len(frames[0]), 10):
        ...
        for pc in pixels_colors:
            current_label = pc[1]
            offline_label = str(int(correct_labels[current_label]))
...
def clusters_to_tracking(centers, labels):
    ...
    for i in range(len(centers)):
        ...
        plane[y, x] = labels_to_color[str(i)]
        if len(older) != 0:
            cv.line(plane, (x, y), older[i], color=labels_to_color[str(i)])
